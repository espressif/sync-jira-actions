...
def run_sync_issue(event_name, event, jira_issue=None):
    ...
    try:
        ...
        event_file = tempfile.NamedTemporaryFile('w+', delete=False)  # pylint: disable=consider-using-with
        ...
        create_autospec(github.Repository.Repository)
        ...
        sync_to_jira._JIRA = jira_class  # pylint: disable=protected-access
...
class TestIssuesEvents(unittest.TestCase):
    def test_issue_opened(self):
        issue = {
            'html_url': 'https://github.com/espressif/fake/issues/3',
    ...
    def test_issue_edited(self):
        issue = {
            'html_url': 'https://github.com/espressif/fake/issues/11',
            'repository_url': 'https://github.com/espressif/fake',
            'number': 11,
            'title': 'Edited issue',
    ...
    def _test_issue_simple_comment(self, action, gh_issue=None):
        """
        Wrapper for the simple case of updating an issue (with 'action').
        GitHub issue fields can be supplied, or generic ones will be used.
        """
        if gh_issue is None:
            gh_number = hash(action) % 43
            gh_issue = {
                'html_url': f'https://github.com/espressif/fake/issues/{gh_number}',
                'number': gh_number,
                'title': 'Test issue',
        ...
        event = {'action': action, 'issue': gh_issue}
    ...
    def test_pr_opened(self):
        pr = {
            'html_url': 'https://github.com/espressif/fake/pulls/4',
...
class TestIssueCommentEvents(unittest.TestCase):
    ...
    def test_issue_comment_edited(self):
        self._test_issue_comment(
            'edited', extra_event_data={'changes': {'body': {'from': 'I am the old comment body'}}}
        )

    def _test_issue_comment(
        self, action, gh_issue=None, gh_comment=None, extra_event_data={}
    ):  # pylint: disable=dangerous-default-value
        """
        Wrapper for the simple case of an issue comment event (with 'action').
        GitHub issue and comment fields can be supplied, or generic ones will be used.
        """
        if gh_issue is None:
        ...
        if gh_comment is None:
            ...
            gh_comment = {
                'html_url': gh_issue['html_url'] + '#' + str(gh_comment_id),
                'repository_url': 'https://github.com/espressif/fake',
                'id': gh_comment_id,
                'user': {'login': 'commentuser'},
                'body': 'ZOMG a comment!',
            }
        event = {'action': action, 'issue': gh_issue, 'comment': gh_comment}
        event.update(extra_event_data)

        m_issue = create_autospec(jira.Issue)(None, None)
        jira_id = hash(action) % 1003
        m_issue.id = jira_id
        m_issue.key = f'FAKEFAKE-{hash(action) % 333}'
